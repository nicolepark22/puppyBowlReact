...
function AddPuppyForm() {
    ...
    async function handleSubmit(evt) {
      ...
      return <div className="addPuppyForm">
  

    <form onSubmit={handleSubmit}>
      <label>
          Name:
        <input id="formInput" value={name} onChange={(evt) => setName(evt.target.value) }/>
      </label>
      <label>
          Breed:
          <input id="formInput" value={breed} onChange={(evt) => setBreed(evt.target.value) }/>
      </label>
      <label>
          Status:
          {/* <input value={status} onChange={(evt) => setStatus(evt.target.value) }/> */}
          {/* <h5>{statusMsg}</h5> */}
          <select id="formInput" onChange={(evt) => setStatus(evt.target.value)}>
      ...
      <label>
          ImageUrl:
          <input id="formInput" value={imageUrl} onChange={(evt) => setImageUrl(evt.target.value) }/>
      </label>

      <button id="formInput" type='submit'>Add New Puppy</button>
    </form>
